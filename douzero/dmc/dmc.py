...
mean_episode_return_buf = {p:deque(maxlen=100) for p in ['landlord', 'farmer']}
...
def train(flags):  
    ...
    for device in device_iterator:
        _free_queue = {'landlord': ctx.SimpleQueue(), 'farmer': ctx.SimpleQueue()}
        _full_queue = {'landlord': ctx.SimpleQueue(), 'farmer': ctx.SimpleQueue()}
    ...
    position_frames = {'landlord':0, 'farmer':0}
    ...
    # for device in device_iterator:
    #     act(0, device, free_queue[device], full_queue[device], models[device], buffers[device], flags)
    # print("act done")
    # return
    ...
    for device in device_iterator:
        for m in range(flags.num_buffers):
            ...
            free_queue[device]['farmer'].put(m)


    threads = []
    locks = {}
    for device in device_iterator:
        locks[device] = {'landlord': threading.Lock(), 'farmer': threading.Lock()}
    position_locks = {'landlord': threading.Lock(), 'farmer': threading.Lock()}
    ...
    try:
        ...
        while frames < flags.total_frames:
            ...
            log.info('After %i (L:%i F:%i) frames: @ %.1f fps (avg@ %.1f fps) (L:%.1f F:%.1f ) Stats:\n%s',
                     frames,
                     position_frames['landlord'],
                     position_frames['farmer'],
                     fps,
                     fps_avg,
